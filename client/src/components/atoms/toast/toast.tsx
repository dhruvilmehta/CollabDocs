...
const Toast=({id, color, title, body, actions}: ToastInterface)=>{
    ...
    return (
        <div onClick={handleToastClick} className={`${TOAST_CLASSES[color]} w-full rounded bg-white dark:bg-slate-700 shadow-md dark:shadow-2xl flex items-stretch text-sm relative cursor-pointer text-primary`}>
             <div className="w-full p-4 space-y-1">
                ...
                <div className="max-w-fit flex flex-wrap justify-start items-start">
                    {actions?.map((a,index)=>{
                        return (<button key={index} className="text-blue-500 font-semibold hover:underline text-center pr-2 action" onClick={()=>a.action()}>
                            {a.label}
                        </button>)
                    })}
                </div>
