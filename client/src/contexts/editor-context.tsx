...
export const EditorProvider=({children}:EditorProviderInterface)=>{
    ...
    useEffect(()=>{
        if(document===null || accessToken===null || socket===null || (socket.current!==null && socket.current.connected)) return 
    ...
    useEffect(()=>{
        ...
        const handler=(currentUsers: Array<string>)=>{
            console.log(currentUsers, "from editor context")
    ...
    return (
        <EditorContext.Provider value={{editorState, socket, documentRendered, setDocumentRendered, editorRef, currentFont, setEditorState, setCurrentFont, focusEditor, handleEditorChange}}>
            {children}
