...
const Register=()=>{
    ...
    const register=async()=>{
        ...
        try{
            ...
            if(axios.isAxiosError(err)){
                ...
                const password2FieldErrors=errors.filter((error:any)=>error.param==='password2').map((error:any)=>error.msg)
                ...
                if(!emailErrors && !password1FieldErrors && !password2FieldErrors) error("An unknown error is occured. Please try again")
            }else{
    ...
    <div
      ...
      style={{ width: widthStr, height: "100%" }}
    >
            <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded border-primary shadow-md border dark:border-0 dark:shadow-xl p-6">
                <div className="flex flex-col space-y-4">
                    ...
                    />
                    <TextField value={password1} onInput={handleOnInputPassword1} label="Password" color="secondary" errors={password1Errors} type="password"
                    />
                    <TextField value={password2} onInput={handleOnInputPassword2} label="Confirm" color="secondary" errors={password2Errors} type="password"
                    />
                    <Link to="/login" className="text-sm hover:underline font-semibold text-blue-500 text-left">
                        Sign in Instead
                    </Link>
                    <button onClick={register} disabled={loading} className="bg-blue-600 text-white text-sm font-semibold px-3 py-2 rounded hover:bg-blue-500 flex justify-center items-center space-x-1 active:ring-1">
