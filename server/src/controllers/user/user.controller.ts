...
class UserController{
    ...
    public verifyEmail=catchAsync(async (req: Request, res: Response)=>{
        ...
        jwt.verify(verificationToken, "verify_email"),
        async(err: VerifyErrors | null, decoded: unknown)=>{
            ...
            try{
                ...
                userService.findUserByVerificationToken(email, verificationToken).then((user)=>{
                    ...
                    userService.updateIsVerified(user, true).then(()=>res.sendStatus(200)).catch(()=>{
    ...
    public confirmResetPassword=catchAsync(async(req: Request, res: Response)=>{
        ...
        jwt.verify(resetPasswordToken, "password_reset", async(err: VerifyErrors | null, decoded: unknown)=>{
            if(err) return res.sendStatus(403)
            try{
                const {email}=decoded as {email: string}
